...
compare.UGP <- function(packages, func, D, N, Npred=1000, reps=1, debug=F, init_list=list(), pred_list=list()) {
  ...
  rmseprmse_plot <- (ggplot2::ggplot(com2, ggplot2::aes_(x=~value, y=~rmseprmse, color=~as.factor(rep)))
                     + ggplot2::geom_point(ggplot2::aes_(shape=~rmseprmse),size=3) + ggplot2::facet_grid(package ~ .)
